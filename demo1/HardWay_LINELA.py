...
model=LinearElasticityNet2D().to(device)
model=model.double()
############################################################
# FEM
...
K_new=torch.tensor([[0],[0]]).to('cuda').double().reshape((2,))
lamdafix=torch.tensor([1]).reshape([1]).double().to('cuda')
mufix=torch.tensor([1]).reshape([1]).double().to('cuda')
parsfuncI=lambda x: torch.cat((Src_new[0:1],Src_new[1:2],K_new),axis=0)
